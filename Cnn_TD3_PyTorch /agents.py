...
class Cnn_Critic(nn.Module):
    def __init__(self, action_dim, batch_size):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(32*6*6+action_dim, 400),
            nn.Tanh(),
            nn.Linear(400, 300),
            nn.Tanh(),
            nn.Linear(300, 200),
...
class TD3():
    def __init__(self, args, action_dim, max_action, state_dim, device):
        ...
        self.actor = Cnn_Actor(action_dim).to(device)
        self.target_actor = Cnn_Actor(action_dim).to(device)
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=args['actor_lr'])

        self.critic1 = Cnn_Critic(action_dim, args['batch_size']).to(device)
        self.target_critic1 = Cnn_Critic(action_dim, args['batch_size']).to(device)
        self.critic2 = Cnn_Critic(action_dim, args['batch_size']).to(device)
        self.target_critic2 = Cnn_Critic(action_dim, args['batch_size']).to(device)
    ...
    def train(self, summary, timestep):
        ...
        target_next_a = self.target_actor(next_states, batch_size) + torch.Tensor(smoothing_noise).to(self.device)
        ...
        actor_loss = -self.critic1(states, self.actor(states, batch_size)).mean()
